...
//std::vector<std::unique_ptr<graph_state>> informative_search(const graph_state &initial, std::function<bool(const graph_state&, const graph_state&)> comp, bool print_stats)
//{
//	std::vector<std::unique_ptr<graph_state>> path;
//	std::unordered_set<std::unique_ptr<graph_state>> closed;
//	updatable_priority_queue<graph_state, decltype(comp)> open {comp};
...
//			path.emplace_back(std::move(s));
//			while (parent_state != nullptr)
//			{	// trzeba wyciągnąć stany z konteneru "closed" i przenieść do "path"
//				std::unique_ptr<graph_state> key {const_cast<graph_state*>(parent_state)};
//				path.emplace_back(std::move(closed.extract(closed.find(key)).value()));
//				key.release();
//				parent_state = parent_state->get_parent();
//			}
//			break;
...
//void informative_search(const graph_state &initial, std::function<bool(const graph_state&, const graph_state&)> comp, std::function<bool(const std::unique_ptr<graph_state>&)> callback, bool print_stats)
//{
//	std::unordered_set<std::unique_ptr<graph_state>> closed;
//	updatable_priority_queue<graph_state, decltype(comp)> open {comp};
